...
def test_invalid_method_request(mock_redis, invalid_method_request):
    for idx, request in enumerate(invalid_method_request):
        ...
        assert code == api.INVALID_REQUEST, (
            f"Request {idx} failed in code in test_invalid_method_request"
        )
        assert len(response) > 0, (
            f"Request {idx} failed in response in test_invalid_method_request"
        )
...
def test_invalid_score_request(mock_redis, invalid_score_request):
    for idx, arguments in enumerate(invalid_score_request):
        ...
        response, code = get_response(mock_redis, request)[:2]
        assert code == api.INVALID_REQUEST, (
            f"Request {idx} failed in code in test_invalid_score_request"
        )
        assert len(response) > 0, (
            f"Request {idx} failed in response in test_invalid_score_request"
        )
...
def test_ok_score_request(mock_redis, ok_score_request):
    for idx, arguments in enumerate(ok_score_request):
        ...
        assert isinstance(score, (int, float)) and score >= 0, (
            f"Request {idx} failed in score in test_ok_score_request"
        )
        assert sorted(context["has"]) == sorted(arguments.keys()), (
            f"Request {idx} failed in context in test_ok_score_request"
        )
...
def test_invalid_interests_request(mock_redis, invalid_interests_request):
    for idx, arguments in enumerate(invalid_interests_request):
        ...
        response, code = get_response(mock_redis, request)[:2]
        assert code == api.INVALID_REQUEST, (
            f"Request {idx} failed in code in test_invalid_interests_request"
        )
        assert len(response) > 0, (
            f"Request {idx} failed in response in test_invalid_interests_request"
        )
...
def test_ok_interests_request(mock_redis, ok_interests_request):
    for idx, arguments in enumerate(ok_interests_request):
        ...
        response, code, _, context = get_response(mock_redis, request)
        assert code == api.OK, (
            f"Request {idx} failed in code in test_ok_interests_request"
        )
        assert len(arguments["client_ids"]) == len(response), (
            f"Request {idx} failed in response in test_ok_interests_request"
        )
        assert all(
        ...
        assert context.get("nclients") == len(arguments["client_ids"]), (
            f"Request {idx} failed in context in test_ok_interests_request"
        )
...
def test_ok_score_request_with_bad_store(mock_redis, bad_store):
    ...
    response, code, _, context = get_response(mock_redis, request, bad_store)
    assert code == api.OK, (
        "Request failed in code in test_ok_score_request_with_bad_store"
    )
    score = response.get("score")
    assert isinstance(score, (int, float)) and score >= 0, (
        "Request failed in score in test_ok_score_request_with_bad_store"
    )
    assert sorted(context["has"]) == sorted(arguments.keys()), (
        "Request failed in context in test_ok_score_request_with_bad_store"
    )
